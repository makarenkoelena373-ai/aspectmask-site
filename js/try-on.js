...
async function createLandmarker(runningMode) {
  // Loaded from jsDelivr instead of self-hosted, added 2026-08-20 during the
  // AR try-on repo merge — the two wasm binaries here (~10-11MB each) exceed
  // the deploy tooling's upload size limit for this project's GitHub-web-upload
  // workflow. Pinned to the exact version (1.0.1) that was vendored locally
  // (file sizes matched byte-for-byte against the CDN listing) so behavior is
  // unchanged. face_landmarker.task and vision_bundle.js stay self-hosted
  // below since they're small enough to upload normally.
